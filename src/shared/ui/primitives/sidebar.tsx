...
function useSidebar() {
	...
	if (!ctx) {
		throw new Error('useSidebar должен быть использован внутри <SidebarProvider>')
	}
		...
		>
			...
			>
				<div
					...
					className='flex size-full flex-col bg-sidebar/25 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:shadow-sm group-data-[variant=floating]:ring-1 group-data-[variant=floating]:ring-sidebar-border'
				>
					{children}
				</div>
			</div>
...
function SidebarGroupLabel({
	...
	test,
	...props
}: useRender.ComponentProps<'div'> & React.ComponentProps<'div'> & { test?: string }) {
	return useRender({
		defaultTagName: 'div',
...
function SidebarGroupAction({
	...
	test,
	...props
}: useRender.ComponentProps<'button'> & React.ComponentProps<'button'> & { test?: string }) {
	return useRender({
...
const sidebarMenuButtonVariants = cva(
	'peer/menu-button group/menu-button flex w-full items-center gap-2 overflow-hidden rounded-md p-2 text-left text-sm outline-hidden ring-sidebar-ring transition-all hover:bg-sidebar-accent hover:text-sidebar-accent-foreground focus-visible:ring-2 active:bg-sidebar-accent active:text-sidebar-accent-foreground disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-active:bg-sidebar-accent data-active:font-medium data-active:text-sidebar-accent-foreground data-open:hover:bg-sidebar-accent data-open:hover:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&_svg]:size-4 [&_svg]:shrink-0',
	{
		variants: {
			variant: {
