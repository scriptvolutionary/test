...
function Sheet({ test, ...props }: SheetPrimitive.Root.Props & { test?: string }) {
	return <SheetPrimitive.Root data-slot='sheet' data-test={test} {...props} />
}

function SheetTrigger({ test, ...props }: SheetPrimitive.Trigger.Props & { test?: string }) {
	return <SheetPrimitive.Trigger data-slot='sheet-trigger' data-test={test} {...props} />
}

function SheetClose({ test, ...props }: SheetPrimitive.Close.Props & { test?: string }) {
	return <SheetPrimitive.Close data-slot='sheet-close' data-test={test} {...props} />
}
...
function SheetOverlay({ className, ...props }: SheetPrimitive.Backdrop.Props) {
	return (
		<SheetPrimitive.Backdrop
			...
			className={cn(
				'data-closed:fade-out-0 data-open:fade-in-0 fixed inset-0 z-50 bg-black/10 duration-200 data-closed:animate-out data-open:animate-in data-ending-style:opacity-0 data-starting-style:opacity-0 supports-backdrop-filter:backdrop-blur-xs',
				className
			)}
			{...props}
		/>
...
function SheetTitle({ className, test, ...props }: SheetPrimitive.Title.Props & { test?: string }) {
	return (
...
function SheetDescription({
	...
	test,
	...props
}: SheetPrimitive.Description.Props & { test?: string }) {
	return (
