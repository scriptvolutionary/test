...
export async function initStoresSync() {
	await Promise.all([useSettingsStore.persist.rehydrate(), useSessionStore.persist.rehydrate()])

	await Promise.all([
		seedIfMissing(useSettingsStore, 'nexus:settings-store'),
		seedIfMissing(useSessionStore, 'nexus:session-store')
	])
}
