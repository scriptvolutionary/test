...
async function invalidateUserDetail(qc: ReturnType<typeof useQueryClient>, id: number) {
...
export function useUpdateUserMutation() {
	...
	return useMutation({
		...
		onSuccess: async (_res, vars) => {
			await Promise.all([invalidateUsersList(qc), invalidateUserDetail(qc, vars.id)])
		}
	})
}
...
export function useBlockUserMutation() {
	...
	return useMutation({
		...
		onSuccess: async (_res, id) => {
			await Promise.all([invalidateUsersList(qc), invalidateUserDetail(qc, id)])
		}
	})
}
