...
import { useCurrentModule, useEnabledModules, useSetModule } from '@/platform/core/hooks'
...
function SidebarModuleMenuSwitcher() {
	...
	return (
		...
		>
			<DropdownMenuGroup>
				<DropdownMenuLabel className='text-muted-foreground text-xs'>Модули</DropdownMenuLabel>
				{modules.map((m) => {
					...
					return (
						...
						>
							...
							<div className='flex flex-col'>
								...
								<p className='text-muted-foreground text-xs'>{m.meta.description}</p>
							</div>
							{isActive && <CheckCheckIcon className='ml-auto text-muted-foreground' />}
						</DropdownMenuItem>
					)
				})}
			</DropdownMenuGroup>
